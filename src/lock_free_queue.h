...
template <class T>
inline size_t lock_free_queue<T>::capacity() const
{
  return m_capacity;
}

template <class T>
inline bool lock_free_queue<T>::enqueue(const T& x)
{
...
template <class T>
inline bool lock_free_queue<T>::enqueue(T && x)
{
  const size_t read_ptr = m_read_ptr.load();
  const size_t write_ptr = m_write_ptr.load();
...
template <class T>
inline bool lock_free_queue<T>::dequeue(T & x)
{
  const size_t read_ptr = m_read_ptr.load();
  const size_t write_ptr = m_write_ptr.load();
